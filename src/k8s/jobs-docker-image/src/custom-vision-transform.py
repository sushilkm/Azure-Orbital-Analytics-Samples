...
if __name__ == "__main__":
    ...
    # Create procesing_dir
    os.mkdir(f'{args.mount_path}/{args.processing_dir}')

    # list the files in the source folder path under the storage account's container
    images_to_mosaic = os.listdir(f'{args.mount_path}/{mosaic_src_folder}')
    ...
    input_path = f'{args.mount_path}/{mosaic_src_folder}'
    output_path = f'{args.mount_path}/{crop_src_folder}'
    # only if there is more than one tif file, 
    # then call mosaic
    if len(input_files) > 1:
        output_path = f'{args.mount_path}/{args.processing_dir}/{crop_src_folder}'
    ...
    output_path = f'{args.mount_path}/{args.processing_dir}/{convert_src_folder}'

    images_to_crop = os.listdir(input_path)
    os.mkdir(output_path)

    input_files = []
    for file in images_to_crop:
        logger.info(f'iterating file in loop with path {file}')
        if not os.path.isdir(file):
            input_files.append(f'{args.mount_path}/{crop_src_folder}/{file}')
    ...
    input_path = output_path
    output_path = f'{args.mount_path}/{args.processing_dir}/{tiles_src_folder}'
    os.mkdir(output_path)
    ...
    input_path = f'{args.mount_path}/{args.processing_dir}/{tiles_src_folder}'
    output_path = f'{args.mount_path}/{args.processing_dir}/{tiles_dst_folder}'
    os.mkdir(output_path)
